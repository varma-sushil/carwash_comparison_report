...
def create_storage_directory_for_historical_data():
    created_path=None
    try:
        path = os.path.join(historical_data_path)
        os.makedirs(path, exist_ok=True)
        print(f"Directory '{path}' created successfully")
        created_path =path
    except OSError as error:
...
def generate_single_historical_report(file_name_with_fullpath,weeks_days_current):
    ...
    monday_date_str, friday_date_str, saturday_date_str, sunday_date_str =  history_format_date_washify(weeks_days_current)
    ...
    #for hamilton dates
    monday_date_str, friday_date_str, saturday_date_str, sunday_date_str = history_format_date_hamilton(weeks_days_current)
...
if __name__=="__main__":
    ...
    from historical_days_gen import *
    ...
    storage_path = create_storage_directory_for_historical_data()
    filename=f"{path}.xlsx"
    file_name_with_fullpath = os.path.join(storage_path,filename)
    ...
    for index,one_week_days in enumerate(result):
        ...
        logger.info(f"completed {index+1}/{total_cnt} ")
